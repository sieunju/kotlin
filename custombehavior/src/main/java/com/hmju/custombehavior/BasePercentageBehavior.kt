package com.hmju.custombehavior
...
/**
 * Class: BasePercentageBehavior
 * Created by jsieu on 2019-08-14.
 *
 * Description: CoordinatorLayout Behavior
 * 기반의 원하는 위치로 스크롤 했을경우 움직이도록 하는 기본 추상화 클래스.
 */
abstract class BasePercentageBehavior<V : View> internal constructor(context: Context, @NonNull attrs: AttributeSet) :
    ...
    init {
        val attr: TypedArray = mContext.obtainStyledAttributes(attrs, R.styleable.CustomBehavior)
        mDependId = attr.getResourceId(R.styleable.CustomBehavior_behavior_dependId, 0)
        mDependPin = attr.getDimension(R.styleable.CustomBehavior_behavior_dependPin, UNSPECIFIED_FLOAT)
        ...
        mDependType = TYPE.values()[attr.getInt(R.styleable.CustomBehavior_behavior_dependType, 1)] // 기본값은 Vertical
        mDependRange = attr.getDimension(
            R.styleable.CustomBehavior_behavior_dependRange,
            UNSPECIFIED_FLOAT
    ...
    open fun onCreate(parent: CoordinatorLayout, child: V, dependency: View) {
        ...
        if (TYPE.VERTICAL == mDependType) {
            ...
            if (mDependPin == UNSPECIFIED_FLOAT) {
                mDependPin = getActionHeight(mContext)
            }

            // 스크롤 범위 최대값 -> Dependency View 높이 - 상태바 높이
            val maxRange: Float = mDependHeight - mDependPin + getStatusBarHeight(mContext)
            // 스크롤 범위 값 제한.
     ...
     * @author hmju
     */
    protected fun getStatusBarHeight(@NonNull ctx: Context): Float {
        // 버전별로 분기 처리.
        return if (Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN) {
            var result = 0F
            val resourceId = ctx.resources.getIdentifier("status_bar_height", "dimen", "android")
            if (resourceId > 0) {
                result = ctx.resources.getDimension(resourceId)
            }
            result
        }
        // 16 미만 0F
    ...
    /**
     * get Default ActionBarSize
     * @param ctx Context
     * @return Default ActionBar Height.
     * @author hmju
     */
    private fun getActionHeight(@NonNull ctx: Context): Float {
        val attrs: TypedArray = ctx.theme.obtainStyledAttributes(
