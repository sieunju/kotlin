 ...
 */
class TranslationBehavior(context: Context, attrs: AttributeSet) : BasePercentageBehavior<View>(context, attrs) {
    ...
    private var mEndXType: TYPE = TYPE.START            // 이동하고 싶은 X 좌표의 기준 (Dependency View 의 X 위치값 기준.)
    private var mEndYType: TYPE = TYPE.START            // 이동하고 싶은 Y 좌표의 기준 (Dependency View 의 Y 위치값 기준.)
    ...
    init {
        ...
        val endX: String? = attr.getString(R.styleable.CustomBehavior_behavior_endX)
        ...
        val endY: String? = attr.getString(R.styleable.CustomBehavior_behavior_endY)
        // endY Null 값이 아닌경우 mEndY 에 알맞게 값 셋팅 한다.
        if (endY != null) {
        ...
        mEndWidth = attr.getDimensionPixelOffset(R.styleable.CustomBehavior_behavior_endWidth, UNSPECIFIED_INT)
        mEndHeight = attr.getDimensionPixelOffset(R.styleable.CustomBehavior_behavior_endHeight, UNSPECIFIED_INT)

        mEndAlpha = attr.getFloat(R.styleable.CustomBehavior_behavior_endAlpha, UNSPECIFIED_FLOAT)
        attr.recycle()
    }
